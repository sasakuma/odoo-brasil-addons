...
class WizardCartaCorrecaoEletronica(models.TransientModel):
    ...
    def send_letter(self):
        ...
        numero_evento = len(self.electronic_doc_id.cartas_correcao_ids) + 1
        carta = {
            'invoice_id': self.electronic_doc_id.id,
            'CNPJ': re.sub("[^0-9]", "", self.electronic_doc_id.company_id.cnpj_cpf or ''),  # noqa: 501
        ...
        if retorno.cStat == 128 and retorno.retEvento.infEvento.cStat in (135,
            ...
            self.write({
                'state': 'error',
                'message': mensagem,
                'sent_xml': base64.b64encode(resposta['sent_xml'].encode('utf8')),
                'sent_xml_name': 'cce-envio.xml',
                'received_xml': base64.b64encode(resposta['received_xml'].encode('utf8')),
